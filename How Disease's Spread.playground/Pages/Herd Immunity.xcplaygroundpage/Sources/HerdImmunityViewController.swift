//
//  HerdImmunityViewController.swift
//  BookCore
//
...
public class HerdImmunityViewController: UIViewController {
    ...
    var recoveredPercentageLabel: UILabel!
    ...
    func setupView() {
        ...
        recoveredPercentageLabel = label
        recoveredPercentageLabel.font = UIFont.boldSystemFont(ofSize: 18)
        recoveredPercentageLabel.text = "Immunity: 0%"
        bottomStackView.addArrangedSubview(recoveredPercentageLabel)
    }
    ...
    func addSimulationDoneView() {
        if fadeView == nil {
            ...
            simulationDoneView = SimulationDoneView(simulationMode: .herdImmunity, peak: maxI, immunity: round(10 * (CGFloat(self.R) / CGFloat(self.N)) * 100.0) / 10.0, beforeTimeUp: t > 0)
    ...
    func progress() {
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                self.recoveredPercentageLabel.text = "Immunity: \(round(10 * (CGFloat(self.R) / CGFloat(self.N)) * 100.0) / 10.0)%"
            }
...
extension HerdImmunityViewController: SimulationDoneDelegate {
